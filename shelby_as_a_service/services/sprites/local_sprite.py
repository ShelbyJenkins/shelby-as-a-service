...
from services.sprites.local_app_theme import AtYourServiceTheme
from services.agents.ceq_agent import CEQAgent
from services.agents.vanilla_agent import VanillaChatAgent
...
class LocalSprite(AppBase):
    ...
    def __init__(self, config_path):
        """ """
        super().__init__(
            service_name_="local_sprite",
            required_variables_=["default_deployment_enabled"],
            config_path=config_path,
        )

        self.ceq_agent = CEQAgent(self.config_path)
        self.vanilla_agent = VanillaChatAgent(self.config_path)
            ...
            with gr.Tab(label="Chat"):
                with gr.Tab(label="Vanilla Chat"):
                    with gr.Column(variant="panel"):
                        vanilla_chatbot = gr.Chatbot(label="Chatbot", lines=20)
                        with gr.Group():
                            with gr.Row():
                                vanilla_message_textbox = gr.Textbox(
                                    container=False,
                                    show_label=False,
                                    label="Message",
                    ...
                    vanilla_message_textbox.submit(
                        fn=self.vanilla_agent.run_query,
                        inputs=vanilla_message_textbox,
                        outputs=vanilla_chatbot,
                    )
