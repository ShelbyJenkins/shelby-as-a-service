...
def parse_retrieved_docs(
    retrieved_documents: list[dict], doc_max_token_length: int = 0, docs_max_total_tokens: int = 0, docs_max_count: int = 0
) -> Optional[list[dict]]:
    """
    ...
    Args:
        ...
        doc_max_token_length (int, optional): The maximum number of tokens allowed for a single document. Documents with
            more tokens will be filtered out. Defaults to 0, which means no filtering based on token count.
        docs_max_total_tokens (int, optional): The maximum total number of tokens allowed for all documents. Documents
    ...
    sorted_documents = [doc for doc in sorted_documents if doc["token_count"] <= doc_max_token_length]

    if len(retrieved_documents) == 1:
        retrieved_documents[0]["doc_num"] = 1
        return retrieved_documents

    if docs_max_total_tokens > 0:
        iterations = 0
        original_documents_count = len(sorted_documents)
        while docs_total_tokens > docs_max_total_tokens:
            if iterations >= original_documents_count:
                break

            # Find the index of the document with the highest token_count that exceeds docs_max_total_tokens
            max_token_count_idx = max(
                (idx for idx, document in enumerate(sorted_documents) if document["token_count"] > docs_max_total_tokens),
...
def old_parse_retrieved_docs(retrieved_documents=None):
    ...
    iterations = 0
    original_documents_count = len(sorted_documents)
    while docs_total_tokens > self.docs_max_total_tokens:
        if iterations >= original_documents_count:
            break
